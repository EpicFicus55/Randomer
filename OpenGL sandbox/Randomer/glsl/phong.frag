...
vec4 _ambient_color = uLightColor * texture( uMaterial.diffuseMap, vTexCoord ) * 0.2f;
...
vec4 _diffuse		= _theta * ( uLightColor * texture( uMaterial.diffuseMap, vTexCoord ) );
...
vec4	_specular = 0.5 * _spec * texture( uMaterial.specularMap, vTexCoord );
